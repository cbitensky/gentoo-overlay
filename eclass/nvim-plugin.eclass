...
# @MAINTAINER:
# cbitensky1@gmail.com
# @SUPPORTED_EAPIS: 7
# @BLURB: used for installing neovim plugins
# @DESCRIPTION:
# This eclass simplifies installation of app-nvim plugins into
# /usr/share/neovim/site/pack.  This is a version-independent directory
# which is read automatically by neovim.  The only exception is
...
NVIM_PLUGIN_NVIM_VERSION="${NVIM_PLUGIN_NVIM_VERSION:-0.5}"

DEPEND=">=app-editors/neovim-${NVIM_PLUGIN_NVIM_VERSION}"
RDEPEND="${DEPEND}"
SLOT="0"
...
# * installs all files in "${ED}"/usr/share/neovim/site/pack.
nvim-plugin_src_install() {
	...
	insinto /usr/share/neovim/site/pack
	local d
	for d in *; do
		[[ -d "${d}" ]] || continue
		doins -r "${d}"
